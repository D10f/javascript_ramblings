...
class PriorityQueue<T extends { toString(): string }> {
  ...
  enqueue(node: HeapNode<T>) {
    ...
    this.heap.push(node);
    this.membership.add(node.value.toString());
  ...
  dequeue() {
    ...
    this.bubbleDown();
    this.membership.delete(node.value.toString());
    return node.value;
  }

  contains(value: T) {
    return this.membership.has(value.toString());
  }
